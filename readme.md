...
Then, go run three microservices: `employee-service`, `product-service`, and `organization-service`, e.g.:
```shell
cd employee-service
mvn spring-boot:run
```
...
- **product-service** - a module containing the second of our sample microservices that allows to perform CRUD operation on in-memory repository of products. It communicates with employee-service. 
